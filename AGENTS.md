...
- **Changelog**: Auto-generated by [git-cliff](https://git-cliff.org/) from conventional commits, grouped by type (feat, fix, chore, docs) via the `cliff.toml` config. Run `pnpm run changelog` to regenerate, or `pnpm run changelog:latest` for the latest tag only.
- **Version source**: `VERSION` file is canonical. Update it before tagging. The `version-propagation.yml` workflow auto-updates `README.md` badge, `CHANGELOG.md` header, and `package.json` `version` field.
