...
describe('GistDetail', () => {
  ...
  describe('bindDetailEvents', () => {
    ...
    beforeEach(() => {
      ...
      container.innerHTML = `
        <div class="gist-detail" data-gist-id="gist-1">
          ...
          <div class="file-content-area" id="file-content-area-gist-1" role="tabpanel" aria-labelledby="tab-gist-1-0">
    ...
    it('switches file content and updates tabindex when a file tab is clicked', async () => {
      const gistStoreModule = await import('../../src/stores/gist-store');
      vi.mocked(gistStoreModule.default.getGist).mockReturnValue({
        id: 'gist-1',
        description: 'Test',
        files: {
          'test.ts': { filename: 'test.ts', language: 'typescript', content: 'c1', rawUrl: 'http://example.com/test.ts', size: 100 },
          'readme.md': { filename: 'readme.md', language: 'markdown', content: 'c2', rawUrl: 'http://example.com/readme.md', size: 50 },
          'other.ts': { filename: 'other.ts', language: 'typescript', content: 'c3', rawUrl: 'http://example.com/other.ts', size: 50 },
        },
        public: true,
        starred: false,
        createdAt: '2026-01-01T00:00:00Z',
