...
test.describe('GistStore Integration', () => {
  ...
  test('should filter gists correctly', async ({ page }) => {
    const results = await page.evaluate(async () => {
      const { default: gistStore } = await import('/src/stores/gist-store.ts');
      // Mock some gists in the store
      (gistStore as any).gists = [
        { id: '1', starred: true, description: 'Starred Gist', files: {} },
        { id: '2', starred: false, description: 'My Gist', files: {} }
      ];
      return {
  ...
  test('should search gists correctly', async ({ page }) => {
    const searchResults = await page.evaluate(async () => {
      const { default: gistStore } = await import('/src/stores/gist-store.ts');
      (gistStore as any).gists = [
        { id: '1', description: 'React Hooks', files: { 'hooks.js': { filename: 'hooks.js' } } },
        { id: '2', description: 'TypeScript Tips', files: { 'tips.ts': { filename: 'tips.ts' } } }
      ];
      return {
  ...
  test('should handle gist updates', async ({ page }) => {
    ...
    const success = await page.evaluate(async () => {
      const { default: gistStore } = await import('/src/stores/gist-store.ts');
      (gistStore as any).gists = [{ id: '1', description: 'Old', files: {} }];
  ...
  test('should handle gist deletion', async ({ page }) => {
    ...
    const success = await page.evaluate(async () => {
      const { default: gistStore } = await import('/src/stores/gist-store.ts');
      (gistStore as any).gists = [{ id: '1', description: 'To Delete', files: {} }];
  ...
  test('should toggle star status', async ({ page }) => {
    ...
    const starred = await page.evaluate(async () => {
      const { default: gistStore } = await import('/src/stores/gist-store.ts');
      (gistStore as any).gists = [{ id: '1', starred: false, description: 'Gist', files: {} }];
