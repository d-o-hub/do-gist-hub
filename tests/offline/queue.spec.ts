...
test.describe('Sync Queue and Offline Operations', () => {
  ...
  test('removePendingWrite deletes operation after successful sync', async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { initIndexedDB, queueWrite, getPendingWrites, removePendingWrite } = await import(
        './src/services/db'
      );
      await initIndexedDB();
  ...
  test('updatePendingWriteError increments retry count and stores error', async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { initIndexedDB, queueWrite, getPendingWrites, updatePendingWriteError } = await import(
        './src/services/db'
      );
      await initIndexedDB();
  ...
  test('queue length is tracked correctly', async ({ page }) => {
    await page.evaluate(async () => {
      const { initIndexedDB, queueWrite, removePendingWrite, getPendingWrites } = await import(
        './src/services/db'
      );
      await initIndexedDB();
